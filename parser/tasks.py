...
def generate_url(feed):
    url = ''
    if feed.get('canonicalUrl', ''):
        url = feed.get('canonicalUrl', '')
    else:
        # Need to save all the alternate links
        alternate = feed.get('alternate', [])
    ...
    for feed in data['items']:
        try:
            ...
            if not created:
                ...
                feed_object.origin_url = generate_url(feed)
                feed_object.save()
            else:
